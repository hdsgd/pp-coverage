...
export class BriefingMateriaisCriativosGamService {
  ...
  private async processMarketingBoardSend(
    ...
    console.log('Enviando briefing GAM para o board de marketing (Fluxo de MKT)...');
    ...
    try {
      ...
      await this.savePreObjectLocally(marketingPreData, `${enrichedFormData.id || 'submission'}_gam_marketing_board_predata`);
    } catch (e) {
      console.warn('Falha ao gerar/salvar pre-data do board de marketing GAM:', e);
    }
    ...
    console.log(`Board de marketing GAM: item criado com ID ${marketingItemId} (primeiro envio).`);
    ...
    try {
      ...
      if (Object.keys(resolvedConnectColumns).length > 0) {
        ...
        await this.saveObjectLocally(
          ...
          `${enrichedFormData.id || 'submission'}_gam_marketing_board_connect_columns`
        );
        ...
        await this.savePreObjectLocally(
          ...
          `${enrichedFormData.id || 'submission'}_gam_marketing_board_second_send_predata`
        );
        ...
        console.log(`Board de marketing GAM: colunas conectar_quadros* atualizadas para item ${marketingItemId} (segundo envio).`);
      }
    } catch (e) {
      console.error('Falha ao atualizar colunas conectar_quadros no board de marketing GAM:', e);
    }
    
    console.log(`Conexão estabelecida: Board Marketing GAM (${marketingItemId}) → Board Principal (${mainBoardItemId})`);
  ...
  private async adjustSubitemsCapacity(subitems: SubitemData[], formData?: FormSubmissionData): Promise<SubitemData[]> {
    ...
    const activeTimeSlots = await this.mondayItemRepository.find({
      where: { board_id: BriefingMateriaisCriativosGamService.TIME_SLOTS_BOARD_ID, status: 'Ativo' },
      order: { name: 'ASC' }
    });
   ...
   * Considera área solicitante:
   * - Agendamentos sempre contam
   * - Reservas da mesma área não contam (permite reuso)
   * - Reservas de outras áreas contam (bloqueia)
   */
